...
func init(st: String = "", data=null) -> bool:
	...
	if states_array == []:
		...
		return false
	for new_state in states_array:
		if new_state is StateInterface3D:
...
#	Debug.dbg("stack size", states_stack.size())
	if state.has_animation:
		if animated_sprite != null and animated_sprite.sprite_frames.has_animation(state.animation):
	...
	#			animation_player.play("RESET")
			if animation_player.has_animation(state.animation):
				animation_player.play.call_deferred(state.animation, state.animation_player_blend_time)
